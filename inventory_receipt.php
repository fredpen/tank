...
<div align="center" id="data-form">
	...
	<form action="" method="POST" id="form1" enctype="multipart/form-data">
		<h3><strong>Receive Product</strong></h3>
		<?php
			...
			if ($op == 'save_inv_recceipt') {
				...
				if ($saverecord == 1) {
					...
					for ($i = 0; $i < $item_count; $i++) {
						...
						$average_cost = ($theitemsavgcost[$i] == 0) ? $thecost[$i] / $theitemsQty[$i] : (($theitemsavgcost[$i] * $theitemsonhand[$i]) + $thecost[$i]) / ($theitemsonhand[$i] + $theitemsQty[$i]);
			...
			<table>
				<tr>
					...
					<td align="right" nowrap="nowrap">
						...
						<input type="button" name="savebutton" id="submit-button" value="Save Receipt" onclick="mysavefunction()">
...
<script>
	...
	function myAddItem() {
		...
		if ($form_productqty == 0) {
			$addrow = $addrow * 0;
			alert('Please Enter Quantity');
		}
		if (isNaN($form_productqty)) {
			$addrow = $addrow * 0;
			alert('Please Enter Numberic Quantity');
		}

		if ($baseprice == 0) {
			$addrow = $addrow * 0;
			alert('Please Enter Landing Cost');
		}
		if (isNaN($baseprice)) {
			$addrow = $addrow * 0;
			alert('Please Enter Numeric Landing Cost');
		}
