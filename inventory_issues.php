...
<div align="center" id="data-form">
	...
	<form action="" method="POST" id="form1" enctype="multipart/form-data">
		<h3><strong>Issue Product</strong></h3>
		<?php
		if ($_SESSION['issueprod'] == 1) {
			...
			$issued_dt = !isset($_REQUEST['issued_dt']) ? date("Y-m-d") : $_REQUEST['issued_dt'];
			...
			// obtain next refno
			$sql_const = "select next_inv_iss_no from const where 1=1";
			//echo $sql_const;
			...
			if ($count_const > 0) {
				...
				if ($row['next_inv_iss_no'] == 99999) {
					$next_refno = 1;
				} else {
					$next_refno = $row['next_inv_iss_no'];
				}
			}
			...
			$refno =  "INVISS" . $refnoday . $refnomonth . $refnoyear . $next_refno;
		...
		?>
				<input type="hidden" id="<?php echo $row['item']; ?>" value="<?php echo $row['avgcost']; ?>" />
				<?php
				$k++;
			}
			...
			if ($op == 'searchkeword') {
				if (trim($keyword) != '') {
					...
					$sql_request = "select *  from vendors where trim(vendorno]) = '$keyword'";
			...
			if ($op == 'save_inv_issues') {
				...
				$sql_checkreceipt = "select * from inv_issues where trim(issue_no) = '$lpo_no'";
				...
				for ($i = 0; $i < $item_count; $i++) {
					...
					if ($count_getaccount > 0) {
						...
						// check if there is enough stock at the sub location
						$sql_check_sub_inventory = " select * from itemsubloc  
							where TRIM(loccd) = '$selectlocation' 
							 and TRIM(item) = '$theitems[$i]' 
							 and TRIM(subloc) = '$thesubloc[$i]'";

						$result_check_sub_inventory = mysqli_query($_SESSION['db_connect'], $sql_check_sub_inventory);
						$count_check_sub_inventory = mysqli_num_rows($result_check_sub_inventory);
						if ($count_check_sub_inventory > 0) {
							$row = mysqli_fetch_array($result_check_sub_inventory);
							$useable_onhand = $row['useable_onhand'];
							if ($useable_onhand < $theitemsQty[$i]) {
								$saverecord = 0;
					?>
								<script>
									$('#item_error').html("<strong>One or more Items not sufficient at the selected Sub Location.</strong>");
								</script>
						<?php
				...
				if (file_exists($source_file)) {
					...
					$supportdoc = md5($lpo_no) . ".pdf";
				}
				if (file_exists($source_picture_file)) {
					$thefileextention = 'jpg';
					$supportdoc = md5($lpo_no) . ".jpg";
				}
				$issued_dt_tosave = substr($issued_dt, 8, 2) . "/" . substr($issued_dt, 5, 2) . "/" . substr($issued_dt, 0, 4);
				if ($saverecord == 1) {

					for ($i = 0; $i < $item_count; $i++) {

						$sql_rec_invent = " insert into inv_issues   
									   (supportdoc,item,qtyissued,avgcost,totalcost,itemdesc,prodcost,tax,taxpercent,  
										  periodmonth,periodyear,loccd,subloc,issue_no,issued_dt,vendorno,company,chartcode,description,issued_by,reason) 
										  values ('$supportdoc','" . $theitems[$i] . "'," . $theitemsQty[$i] . " ," . $thebaseprice[$i] . " ," .
							$thecost[$i] . " ,'" . $theitemsDescription[$i] . "'," . $theprice[$i] . " ," . $thetax[$i] .
							",$vat, '$periodmonth','$periodyear','$selectlocation','" .
							$thesubloc[$i] . "','$lpo_no','$issued_dt_tosave','$vendorno','$company','" . $theitemschartcode[$i] .
							"','" . $theitemschartdescription[$i] . "','$reqst_by','$thereason')";

						$result_rec_invent = mysqli_query($_SESSION['db_connect'], $sql_rec_invent);

						//echo "<br />".$sql_rec_invent;
						$average_cost = ($theitemsavgcost[$i] == 0) ? $thecost[$i] / $theitemsQty[$i] : (($theitemsavgcost[$i] + ($thecost[$i] / $theitemsQty[$i])) / 2);
						...
						$sql_const_next_inv_iss_no = " update const set 
								  next_inv_iss_no = next_inv_iss_no + 1 where 1=1";

						$result_const_next_inv_iss_no = mysqli_query($_SESSION['db_connect'], $sql_const_next_inv_iss_no);

						// ** creating journal entry
						$sql_CreateJournal2 = "call CreateJournalEntries('INVENTORYISS','" . $theitemschartcode[$i] . "','" . $theitemschartdescription[$i] . ' - ' . $theitemsDescription[$i] . "'," . $thecost[$i] . ",'" . date('d/m/Y H:i:s A') . "','$periodmonth','$periodyear')";
					...
					$dbobject->apptrail($reqst_by, 'Inventory', $lpo_no, date('d/m/Y H:i:s A'), 'Inventory Issues');

					echo "<h3>The Inventory Issues Number is <b>$lpo_no</b></h3>";
				?>
					<script>
			...
			for ($k = 0; $k < $count_itemsubloc; $k++) {
				$sub_row = mysqli_fetch_array($result_itemsubloc);
				$itemsubloc_var[$k]['loccd'] = $sub_row['loccd'];
				$itemsubloc_var[$k]['item'] = $sub_row['item'];
				$itemsubloc_var[$k]['subloc'] = $sub_row['subloc'];

				?>
				<input type="hidden" name="<?php echo 'subloc' . $itemsubloc_var[$k]['loccd'] . $itemsubloc_var[$k]['item'] . $itemsubloc_var[$k]['subloc']; ?>" id="<?php echo 'subloc' . $itemsubloc_var[$k]['loccd'] . $itemsubloc_var[$k]['item'] . $itemsubloc_var[$k]['subloc']; ?>" value="" />
			...
			<input type="hidden" name="get_file" id="get_file" value="inventory_issues" />
			<div id="items_in_cart"></div>
			<input type="hidden" name="operation" id="operation" value="<?php echo $op; ?>" />
			<table border="0" cellpadding="6px" style="border-spacing: 0 5px;border:1px solid black;padding:1px;border-collapse:separate;border-radius:15px">
				<tr>
				...
				<tr>
					...
					<td>
						...
						$k = 0;
						?>
						<br /><select name="selectvendor" id="selectvendor">
							<option value=""></option>
							<?php
					...
					<td valign="bottom"><strong>Location:</strong>
					</td>
					<td>
						<?php
						$k = 0;
						?>
						<br /><select name="selectlocation" id="selectlocation" onChange="getsublocation()">
							<option value=""></option>
							<?php
				...
				<tr>
					...
					<td><input type="text" name="lpo_no" id="lpo_no" value="<?php echo $lpo_no; ?>"> </td>
					<td> <b>Date</b></td>
					<td> <input type="date" name="issued_dt" id="issued_dt" value="<?php echo $issued_dt; ?>"></td>

				</tr>
				<tr>
					<td><b>Reason</b></td>
					<td colspan="3"><input type="text" size="62%" id="thereason" name="thereason" placeholder="Please state the reason for the return" /></td>
				</tr>

			</table>
			<br />
			<input type="hidden" min="0" max="20" step="0.01" id="taxpercent" name="taxpercent" value="0.00" />
			<div style="overflow-x:auto;">
				<table border="0" cellpadding="4px" style="border:1px solid black;padding:1px;border-collapse:separate;border-radius:15px">
					<tr>
						<td>
							<b>Select Product</b>&nbsp;&nbsp;
						</td>
						<td align="center">
							...
							$k = 0;
							?>
							<br /><select name="selectproduct" id="selectproduct" onChange="javascript: checksubloc();">
								<option value=""></option>
								<?php
						...
						<td align="right"><strong>Sub Loc :</strong></td>
						<td colspan="2" align="center">
							<?php
							$k = 0;
							?>
							<br /><select name="selectitemsubloc" id="selectitemsubloc" onChange="checksubloc()">
								<option value=""></option>
								<?php
					...
					<tr>
						...
						<td>
							<input type="number" size="5px" id="productqty" name="productqty" min="0" step="0.01" />
						...
						<td>
							<input type="text" size="8px" id="baseprice" tabindex="-1" name="baseprice" readonly />
			...
			<div style="overflow-x:auto;">
				<table border="0" cellpadding="2px" cellspacing="2px" class="menu_backcolor" id="productlistTable">
					<thead>
			...
			<table>
				<tr>
					...
					<td align="right" nowrap="nowrap">
						<label>
							<input type="button" name="savebutton" id="submit-button" value="Save Issues" onclick="mysavefunction()">
						</label>
					</td>
					...
					<td>
						<a href="javascript:getpage('lookupreport.php?reportname=inv_issues','page')">List Transactions</a>
					</td>
...
<script>
	...
	function getsublocation() {
		...
		var $form_selectvendor = $('#selectvendor').val();

		getpage('inventory_issues.php?op=getsublocation&selectlocation=' + $form_selectlocation + '&keyword=' + $form_selectvendor, 'page');
	}
	...
	function checksubloc() {
		...
		var $theavgcost = $('#' + $itemcode).val();
		var baseprice = document.getElementById("baseprice");
		baseprice.value = $theavgcost;
	...
	function myAddItem() {
		...
		if ($form_selectitemsubloc == '') {
			$addrow = $addrow * 0;
			alert('Please Select Sub Inventory');
		}
		if ($vat == '') {
			$addrow = $addrow * 0;
		}
	...
	function mysavefunction() {
		if (confirm('Are you sure the entries are correct?')) {
			...
			var $issued_dt = $('#issued_dt').val();
			var $form_issued_dt = new Date($issued_dt);
			//var today = new Date();
			var thismonth = ($form_issued_dt.getMonth() + 1);
			var thisyear = $form_issued_dt.getFullYear();
			...
			if ($goahead == 1) {
				...
				getpagestring = 'inventory_issues.php?op=save_inv_issues';
				var $vat = $('#taxpercent').val();
				var $form_selectlocation = $('#selectlocation').val();


				getpagestring = getpagestring + '&selectlocation=' + $form_selectlocation + '&vat=' + $vat;


				getpagestring = getpagestring + '&vendorno=' + $form_vendor + '&lpo_no=' + $form_lpo_no + '&thereason=' + $form_thereason;
				getpagestring = getpagestring + '&item_count=' + item_count + '&issued_dt=' + $issued_dt;
				...
				alert('Cannot Save This Product Issue');
			}
