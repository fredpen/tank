...
excludedomains: ["cssdrive.com", "google.com"],
includekeywords: ["transport.php"],
...
toggleHTML: '<form><input type="checkbox" id="targetcheckbox" checked="checked" /><label for="targetcheckbox"> Open off-site links in new (shared target) window?</label></form>',
...
assigntarget:function(){
	...
	var reincludekeywords=new RegExp(this.includekeywords.join("|"), "i")
	var all_links=document.getElementsByTagName("a")
	if (this.mode=="auto" || (this.mode=="manual" && this.togglebox.checked)){
		for (var i=0; i<=(all_links.length-1); i++){
			if (all_links[i].hostname.search(rexcludedomains)==-1 && all_links[i].protocol.indexOf("http:")!=-1 || all_links[i].pathname.search(reincludekeywords)!=-1)
