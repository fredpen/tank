			...
			<div style="overflow-x:auto;">
				<table border="0" style="border:1px solid black;padding:1px;border-collapse:separate;border-radius:15px">
					<tr>
						...
						<td>
							<input type="text" name="meterid" id="meterid" value="<?php echo $meterid; ?>" <?php if ($meterid != '') {
																												echo 'readonly';
																											} ?> placeholder="Enter Gantry Meter ID" />
						</td>
						<td>

							<input type="button" name="refreshbutton" id="submit-button" value="Refresh" onclick="javascript:  getpage('gantrymeters.php?','page');" />
				...
				<table>
					...
					<tr>
						...
						<td>
							...
							<input type="button" name="deletebutton" id="submit-button" value="Delete" onclick="deletemeterid();" />

						</td>

						<td>

							<input type="button" name="savebutton" id="submit-button" value="Save" onclick="savemeterid();" />
...
<script>
	function savemeterid() {
		if (confirm('Are you sure the entries are correct?')) {
			...
			$form_meterid = $('#meterid').val();
			var $meterlabel = $('#meterlabel').val();
