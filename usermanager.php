...
<div align ="center" id="data-form" > 
	...
	<form action="" method="get" id="form1">
		<h3><strong><font size='12'>Bizmaap Users </font></strong></h3>
		<?php
			...
			if($op=='searchkeyword')
			{
				...
				if ($count_userid >=1)
				{
					...
					$sysuserid = $row['userid']; 	$names = $row['names']; 	$roleid = $row['roleid'];
					$administra = $row['administra']; $request = $row['request']; $reports = $row['reports'];
					$productionreq = $row['productionreq']; $productionappr = $row['productionappr']; 
					$delivery = $row['delivery']; $othincome = $row['othincome']; 	$expense = $row['expense'];
					$authentica = $row['authentica']; 
					$pomasters = $row['pomasters']; $somasters = $row['somasters'];$armasters = $row['armasters'];
					$glmasters = $row['glmasters']; $ivmasters = $row['ivmasters'];$apmasters = $row['apmasters'];
					$createpo = $row['createpo']; $receivepo = $row['receivepo'];	$approvepo = $row['approvepo']; $returnpo = $row['returnpo'];
					$glposting = $row['glposting'];
					$receiveprod = $row['receiveprod'];$issueprod = $row['issueprod'];
					$retail = $row['retail'];
					$credits = $row['credits'];
					$ownuse = $row['ownuse'];
					$ptn = $row['ptn'];
					...
					$crrecon = $row['crrecon'];
					$prnrcpt = $row['prnrcpt'];
					$genloadingslip = $row['genloadingslip'];$prnloadingslip = $row['prnloadingslip'];
					$genwaybill = $row['genwaybill'];$prnwaybill = $row['prnwaybill'];
					$reprint = $row['reprint'];
					$storagereadings = $row['storagereadings'];$approvereadings = $row['approvereadings'];
				...
				}else 	
				{
					...
					$createpo = 0;
					$productionreq = 0;
					$productionappr = 0;$storagereadings = 0;$approvereadings = 0;
			...
			if($op=='saving'){
				...
				if ($goahead==1)
				{
					...
					if ($count_userid >=1)
					{
						...
						?>	<script>$('#item_error').html("<strong>Record Updated</strong>");</script>	<?php
					}
					else 	
					{
						...
						?>	<script>$('#item_error').html("<strong>New Record Created</strong>");</script>	<?php
					}
	  ...
	  return $data;
		...
		<table border="0" style="border-spacing: 10px" >
			...
			<tr>
				<td colspan="4" align="left"><br/><b>Business Category :</b>
				 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="checkbox" id="retail" <?php if ($retail==1){echo 'checked';} ?> />&nbsp;&nbsp;<b>Cash</b>
				 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="checkbox" id="credits" <?php if ($credits==1){echo 'checked';} ?> />&nbsp;&nbsp;<b>Credit</b>
				 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="checkbox" id="ownuse" <?php if ($ownuse==1){echo 'checked';} ?> />&nbsp;&nbsp;<b>Own Use</b>
				 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="checkbox" id="ptn" <?php if ($ptn==1){echo 'checked';} ?> />&nbsp;&nbsp;<b>Product Transfer</b>
			...
			<tr>
				<td align="center" colspan="4">
					<table border="0" cellspacing="5" style="outline-style: double;">
						<tr>
							<td><input type="checkbox" id="createpo" <?php if ($createpo==1){echo 'checked';} ?> />&nbsp;&nbsp;<b>Create PO</b>
							</td>
							<td><input type="checkbox" id="approvepo" <?php if ($approvepo==1){echo 'checked';} ?> />&nbsp;&nbsp;<b>Approve PO</b>
							</td>
							<td><input type="checkbox" id="receivepo" <?php if ($receivepo==1){echo 'checked';} ?> />&nbsp;&nbsp;<b>Receive PO</b>
							</td>
							<td><input type="checkbox" id="returnpo" <?php if ($returnpo==1){echo 'checked';} ?> />&nbsp;&nbsp;<b>Return PO</b>
							</td>
						</tr>
						<tr>
							<td><input type="checkbox" id="receiveprod" <?php if ($receiveprod==1){echo 'checked';} ?> />&nbsp;&nbsp;<b>Receive Product</b>
							</td>
							<td ><input type="checkbox" id="issueprod" <?php if ($issueprod==1){echo 'checked';} ?> />&nbsp;&nbsp;<b>Issue Product</b>
							</td>
							<td><input type="checkbox" id="productionreq" <?php if ($productionreq==1){echo 'checked';} ?> />&nbsp;&nbsp;<b>Production Req</b>
							</td>
							<td><input type="checkbox" id="productionappr" <?php if ($productionappr==1){echo 'checked';} ?> />&nbsp;&nbsp;<b>Production Approval</b>
							</td>	
						</tr>
						...
						<tr>
							...
							<td>
								<input type="checkbox" id="request" <?php if ($request==1){echo 'checked';} ?> />&nbsp;&nbsp;<b>Requisition</b>
							</td>
							<td><input type="checkbox" id="approval" <?php if ($approval==1){echo 'checked';} ?> />&nbsp;&nbsp;<b>Approve Requisition</b>
							</td>
							<td>
								<input type="checkbox" id="genloadingslip" <?php if ($genloadingslip==1){echo 'checked';} ?> />&nbsp;&nbsp;<b>Generate Loading Slip</b>
							</td>
							<td>
								<input type="checkbox" id="prnloadingslip" <?php if ($prnloadingslip==1){echo 'checked';} ?> />&nbsp;&nbsp;<b>Print Loading Slip</b>
							</td>
						...
						<tr>
							<td>
								<input type="checkbox" id="genwaybill" <?php if ($genwaybill==1){echo 'checked';} ?> />&nbsp;&nbsp;<b>Generate Waybill</b>
							</td>
							<td>
								<input type="checkbox" id="prnwaybill" <?php if ($prnwaybill==1){echo 'checked';} ?> />&nbsp;&nbsp;<b>Print Waybill</b>
							</td>
							<td><input type="checkbox" id="prninv" <?php if ($prninv==1){echo 'checked';} ?> />&nbsp;&nbsp;<b>Print Invoice</b></td>
							<td >
								<input type="checkbox" id="delivery" <?php if ($delivery==1){echo 'checked';} ?> />&nbsp;&nbsp;<b>Confirm Delivery</b>
							</td>
							
						</tr>
						<tr>
							
							<td><input type="checkbox" id="payments" <?php if ($payments==1){echo 'checked';} ?> />&nbsp;&nbsp;<b>Payments</b>
							</td>
							<td><input type="checkbox" id="prnrcpt" <?php if ($prnrcpt==1){echo 'checked';} ?> />&nbsp;&nbsp;<b>Print Receipt</b></td>
							<td><input type="checkbox" id="reprint" <?php if ($reprint==1){echo 'checked';} ?> />&nbsp;&nbsp;<b>Re-Print</b>
							</td>
							<td><input type="checkbox" id="othincome" <?php if ($othincome==1){echo 'checked';} ?> />&nbsp;&nbsp;<b>Other Income</b>
							</td>
							
						</tr>
						<tr>
							<td><input type="checkbox" id="expense" <?php if ($expense==1){echo 'checked';} ?> />&nbsp;&nbsp;<b>Expenditures</b>
							</td>
							<td><input type="checkbox" id="cca" <?php if ($cca==1){echo 'checked';} ?> />&nbsp;&nbsp;<b>Bank Reconciliation</b>
							</td>
							<td ><input type="checkbox" id="crrecon" <?php if ($crrecon==1){echo 'checked';} ?> />&nbsp;&nbsp;<b>Credit Reconciliation</b>
							</td>
							<td><input type="checkbox" id="directjournal" <?php if ($directjournal==1){echo 'checked';} ?> />&nbsp;&nbsp;<b>Journal Entry</b>
							</td>
						</tr>
						
						<tr>
							<td><input type="checkbox" id="glposting" <?php if ($glposting==1){echo 'checked';} ?> />&nbsp;&nbsp;<b>GL Posting</b>
							</td>
							<td><input type="checkbox" id="somasters" <?php if ($somasters==1){echo 'checked';} ?> />&nbsp;&nbsp;<b>SO Masters</b>
							</td>
							
							<td><input type="checkbox" id="armasters" <?php if ($armasters==1){echo 'checked';} ?> />&nbsp;&nbsp;<b>AR Masters</b>
							</td>
							<td><input type="checkbox" id="pomasters" <?php if ($pomasters==1){echo 'checked';} ?> />&nbsp;&nbsp;<b>PO Masters</b>
							</td>
							
						</tr>
						<tr>
							<td><input type="checkbox" id="glmasters" <?php if ($glmasters==1){echo 'checked';} ?> />&nbsp;&nbsp;<b>GL Masters</b>
							</td>
							<td><input type="checkbox" id="apmasters" <?php if ($apmasters==1){echo 'checked';} ?> />&nbsp;&nbsp;<b>AP Masters</b>
							</td>
							<td><input type="checkbox" id="ivmasters" <?php if ($ivmasters==1){echo 'checked';} ?> />&nbsp;&nbsp;<b>IV Masters</b>
							</td>
							<td ><input type="checkbox" id="storagereadings" <?php if ($storagereadings==1){echo 'checked';} ?> />&nbsp;&nbsp;<b>Storage Readings</b>
							</td>
							
						</tr>
						<tr>
							<td ><input type="checkbox" id="approvereadings" <?php if ($approvereadings==1){echo 'checked';} ?> />&nbsp;&nbsp;<b>Approve Readings</b>
							</td>
							<td colspan="3" ><input type="checkbox" id="reports" <?php if ($reports==1){echo 'checked';} ?> />&nbsp;&nbsp;<b>Reports</b>
							</td>
						</tr>
					</table>
				</td>
			 ...
			 <tr>
				...
				<td nowrap="nowrap" align="right" valign="top" colspan="3">
					<input type="checkbox" id="initializepassword" 
						onClick="javasscript: var thepassword = document.getElementById('password');
		...
		<tr>
			...
			<td  align="center" colspan="2">
				<label>
				  <input type="button" name="deleteuser" id="submit-button" value="Delete" 
					onclick="javascript: 
			...
			<td  align="center" colspan="2">
				<label>
				  <input type="button" name="testsave" id="submit-button" value="Save" 
					onclick="javascript: 
