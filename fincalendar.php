...
<style>
	...
	<form action="" method="get" id="form1">
		<p ><h3><strong><font size='5'>Financial Calendar </font></strong></h3></p>
		<?php
			...
			if($op=='closeopenyear')
			{
				...
				if ($goahead==1)
				{
					...
					if ($row['periodstatus']==0){ $periodstatustxt = 'Closed';$periodstatus = 1;}else{ $periodstatustxt = 'Opened';$periodstatus = 0;} 
					...
					?>	<script>$('#item_error').html("<strong>Financial Year Status Modified Successfully.</strong>");</script>	<?php
			...
			if($op=='addyear')
			{
				...
				if ($goahead==1)
				{
					...
					?>	<script>$('#item_error').html("<strong>Financial Year Added Successfully.</strong>");</script>	<?php
...
//****			
		...
		<table border="0"  style="border:1px solid black;padding:1px;border-collapse:separate;border-radius:15px">
			...
			<tr>
				...
				<td>
					<input type="button" name="addyear" id="submit-button" value="Add Year" 
							onclick="javascript: my_addyear();">
				</td>
			</tr>
			<tr>
				
				<td colspan="2" >
					<?php $calledby = 'fincalendar'; $reportid = 65; include("specificreportlink.php");  ?>
				</td>
				...
				<?php 
					...
					?>
						<tr >
							...
							<td >
								<input type="button" name="<?php echo $row_fincalendar['acctyear']; ?>" id="submit-button" value="<?php if ($row_fincalendar['PeriodStatus'] == 'Closed') {echo 'Open Year';} else { echo 'Close Year';}?>" 
