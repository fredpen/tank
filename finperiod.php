...
<style>
	...
	<form action="" method="get" id="form1">
		<p ><h3><strong><font size='5'>Set Accounting Period </font></strong></h3></p>
		<?php
			...
			{
				$goahead=1;
				if (trim($_REQUEST['periodmonth']) ==''){$goahead=0;
					?>	
						<script>
							$('#item_error').html("<strong>Please specify the Month</strong>");
						</script>
					<?php }
				if (trim($_REQUEST['periodyear']) ==''){$goahead=0;
					?>	
						<script>
				...
				if ($goahead==1)
				{
					...
					?>	<script>$('#item_error').html("<strong>Transaction Period Set Successfully.</strong>");</script>	<?php
...
//****			
		...
		<input type="hidden" name="operation" id="operation" value="<?php echo $op; ?>" />
			...
			<tr>
				<td colspan="2">
					<input type="button" name="Setperiod" id="submit-button" value="Set Period" 
