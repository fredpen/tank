...
//Search box value assigning to $Name variable.
   ...
   $tablename = $dbobject->test_input($_POST['thetablename']);
	switch ($tablename)
	{
		...
		case 'approvals':
			$filter=" (custno like '%$keyword%' || ccompany like '%$keyword%' || request like '%$keyword%')  && trantype=$trantype ";
			$source = 'headdata';
		...
		case 'vendorpay':
			$filter=" inv_recpt_no like '%$keyword%'  || company like '%$keyword%' || vendorno like '%$keyword%' || purchaseorderno like '%$keyword%' ";
			$source = 'accountpayable';
			$fields = " inv_recpt_no as field1,company as field2 ,vendorno as field3,purchaseorderno as field4 ";
		...
		case 'catg':
			$filter=" catcd like '%$keyword%' || catdesc like '%$keyword%'  ";
			$source = 'catg';
			$fields = "catcd as field1,catdesc as field2 ,'' as field3,'' as field4 ";
			
			break;	
			
		case 'meters':
			$filter=" meterid like '%$keyword%' || meterlabel like '%$keyword%'  || loccd like '%$keyword%'  ";
			$source = 'meters';
			$fields = "meterid as field1,meterlabel as field2 ,loccd as field3,'' as field4 ";
...
<ul>
   ...
   while ($Result = mysqli_fetch_array($ExecQuery)) {
	   ...
	   <li onclick='fill("<?php echo $Result['field1']; ?>")'>
