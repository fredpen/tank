...
/*	
	$query = "SELECT '' forgroup, '' acctno, '---ASSETS---' acctdescription, 0 amount,'' accounttype FROM DUAL ".
 			" UNION ".
 			" SELECT '' forgroup,a.acctno, a.acctdescription,   ABS(sum(a.dr_amount-a.cr_amount)) amount,  b.accounttype ".
 			"  FROM ledger a, chart_of_account b where trim(a.acctno) = trim(b.chartcode) and trim(b.accounttype) = 'Asset' and b.bs_stmt = 1  ".
 			" and periodyear = '" . $periodyear . "' ".
 			" 	GROUP BY a.acctno, a.acctdescription, a.periodyear ".
 			" UNION ".
 			" SELECT '' forgroup,'' acctno, '________________________________________' acctdescription, 0 amount,'' accounttype FROM DUAL ".
 			" UNION ".
 			" SELECT '' forgroup,'' acctno, '---LIABILTIES---' acctdescription, 0 amount, '' accounttype FROM DUAL ".
 			" UNION ".
 			" SELECT '' forgroup,a.acctno, a.acctdescription, ABS(sum(a.dr_amount-a.cr_amount)) amount ,b.accounttype ".
 			"  FROM ledger a, chart_of_account b where trim(a.acctno) = trim(b.chartcode) and trim(b.accounttype) = 'Liability' and b.bs_stmt = 1 ".
 			" and periodyear = '" . $periodyear . "' ".
 			" 	GROUP BY a.acctno, a.acctdescription, a.periodyear ";
...
*/
		...
		$query = "SELECT '' forgroup, '' acctno, '---ASSETS---' acctdescription, 0 amount,'' accounttype FROM DUAL ".
 			" UNION ".
 			" SELECT '' forgroup,a.acctno, a.acctdescription,   ABS(sum(a.dr_amount-a.cr_amount)) amount,  b.accounttype ".
 			"  FROM ledger a, chart_of_account b where trim(a.acctno) = trim(b.chartcode) and trim(b.accounttype) = 'Asset'  ".
 			" and periodyear = '" . $periodyear . "' ".
 			" 	GROUP BY a.acctno, a.acctdescription, a.periodyear ".
 			" UNION ".
 			" SELECT '' forgroup,'' acctno, '________________________________________' acctdescription, 0 amount,'' accounttype FROM DUAL ".
 			...
 			" UNION ".
 			" SELECT '' forgroup,a.acctno, a.acctdescription, ABS(sum(a.dr_amount-a.cr_amount)) amount ,b.accounttype ".
 			"  FROM ledger a, chart_of_account b where trim(a.acctno) = trim(b.chartcode) and trim(b.accounttype) = 'Liability'  ".
 			" and periodyear = '" . $periodyear . "' ".
 			" 	GROUP BY a.acctno, a.acctdescription, a.periodyear ";
			...
			$summary_amount1 = 0;$summary_amount2 = 0;
			if ($numrows>0)
			{
				...
				if ($numrows_summary1 > 0){$summary_amount1 = $summary_amount1 + $row_summary1['amount'];}
				if ($numrows_summary2 > 0){$summary_amount2 = $summary_amount2 + $row_summary2['amount'];}
	...
	<form name="form1" id="form1" method="post" action="">
		...
		<h3  class= "noprint"><strong><font size='10'>Balance Sheet</font></strong></h3>
		<h3  class= "noprint"><strong><font size='5'>As at 31st December <?php echo $periodyear;?> </font></strong></h3>
		<a href="#" class="noprint" onclick="window.close();return false"><h3>Close Window</h3></a>
		
		
		<div id="print_table">
			<h3><strong><font size='10'>Balance Sheet </font></strong></h3>
							...
							<table  border="0" cellpadding="5" cellspacing="1" class="menu_backcolor" id="userlistTable">
								...
								?>
										...
										<tr <?php echo ($k%2==0)?"class='treven'":"class='trodd'"; ?> >
											...
											<td nowrap="nowrap" align="right"><?php if ($row["amount"]>0) {echo number_format($row["amount"],2);}?></td>
										<td nowrap="nowrap"></td>
										</tr>
								<?php 
										//} //End For Loop
