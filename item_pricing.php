			...
			<div style="overflow-x:auto;">
				<table border="0" style="border:1px solid black;padding:1px;border-collapse:separate;border-radius:15px">
					<tr>
						...
						<td>
							...
							<input type="button" name="refreshbutton" id="submit-button" value="Refresh" onclick="javascript:  getpage('item_pricing.php?','page');" />
				...
				<div id="showdiscountdiv" style="display:none">
					<table border="0" style="border:1px solid black;padding:1px;border-collapse:separate;border-radius:15px">
						...
						<tr>
							<td nowrap="nowrap">
								<input type="radio" name="retail" id="cshflat" <?php if ($retail == 1) {
																					echo 'checked';
																				} ?> onClick="javascript: refreshoption(this.name);" />Flat
								&nbsp;&nbsp;<input type="radio" name="retail" id="cshpercent" <?php if ($retail == 3) {
																									echo 'checked';
																								} ?> onClick="javascript: refreshoption(this.name);" />Percent
								&nbsp;&nbsp;<input type="radio" name="retail" id="cshslab" <?php if ($retail == 2) {
																								echo 'checked';
																							} ?> onClick="javascript: refreshoption(this.name);" />Slab
								&nbsp;&nbsp;<b>Retail</b>
							</td>
							<td>

								<input type="number" id="retailflat" name="retailflat" value="<?php echo $retflat; ?>" min="0" max="999999" <?php if ($retail != 1) {
							...
							<td>
								...
								?>
								<select name="retailcent" id="retailcent" <?php if ($retail != 3) {
																				echo 'style="display:none"';
																			} ?>>
									<option value=""></option>
									<?php
							...
							<td>
								...
								<select name="retailslab" id="retailslab" <?php if ($retail != 2) {
																				echo 'style="display:none"';
																			} ?>>
									<option value=""></option>
									<?php
						...
						<tr>
							<td nowrap="nowrap">
								<input type="radio" name="ow" id="ownflat" <?php if ($ow == 1) {
																				echo 'checked';
																			} ?> onClick="javascript: refreshoption(this.name);" />Flat
								&nbsp;&nbsp;<input type="radio" name="ow" id="owncent" <?php if ($ow == 3) {
																							echo 'checked';
																						} ?> onClick="javascript: refreshoption(this.name);" />Percent
								&nbsp;&nbsp;<input type="radio" name="ow" id="ownslab" <?php if ($ow == 2) {
																							echo 'checked';
																						} ?> onClick="javascript: refreshoption(this.name);" />Slab
								&nbsp;&nbsp;<b>Own Use</b>
							...
							<td>
								<input type="number" id="owflat" name="owflat" value="<?php echo $owflat; ?>" min="0" max="999999" <?php if ($ow != 1) {
																																		echo 'style="display:none"';
																																	} ?> />
							</td>
							<td>
								...
								$k = 0.00;
								?>
								<select name="owcent" id="owcent" <?php if ($ow != 3) {
																		echo 'style="display:none"';
																	} ?>>
									<option value=""></option>
									<?php
							...
							<td>
								...
								<select name="owslab" id="owslab" <?php if ($ow != 2) {
																		echo 'style="display:none"';
																	} ?>>
									<option value=""></option>
									<?php
						...
						<tr>
							<td nowrap="nowrap">
								<input type="radio" name="pr" id="proflat" <?php if ($pr == 1) {
																				echo 'checked';
																			} ?> onClick="javascript: refreshoption(this.name);" />Flat
								&nbsp;&nbsp;<input type="radio" name="pr" id="procent" <?php if ($pr == 3) {
																							echo 'checked';
																						} ?> onClick="javascript: refreshoption(this.name);" />Percent
								&nbsp;&nbsp;<input type="radio" name="pr" id="proslab" <?php if ($pr == 2) {
																							echo 'checked';
																						} ?> onClick="javascript: refreshoption(this.name);" />Slab
								&nbsp;&nbsp;<b>Default</b>
							...
							<td>
								<input type="number" id="prflat" name="prflat" value="<?php echo $prflat; ?>" min="0" max="999999" <?php if ($pr != 1) {
																																		echo 'style="display:none"';
																																	} ?> />
							</td>
							<td>
								...
								$k = 0.00;
								?>
								<select name="prcent" id="prcent" <?php if ($pr != 3) {
																		echo 'style="display:none"';
																	} ?>>
									<option value=""></option>
									<?php
							...
							<td>
								...
								<select name="prslab" id="prslab" <?php if ($pr != 2) {
																		echo 'style="display:none"';
																	} ?>>
									<option value=""></option>
									<?php
				...
				<table>
					...
					<tr>
						...
						<td>
							<input type="button" name="closebutton" id="submit-button" value="Back" onclick="javascript:  getpage('item_master.php?','page');" />
						</td>
						<td nowrap="nowrap">
							<label>
								<input type="button" name="itemsetting" id="submit-button" value="Item Settings" onclick="javascript: getpage('item_settings.php?','page');">
							</label>
						</td>

						<td>

							<input type="button" name="showthediscount" id="submit-button" value="Show Discount" onclick="javascript:  showdiscount();" />
...
<script>
	...
	function saveicitem() {
		if (confirm('Are you sure the entries are correct?')) {
			...
			var $form_scprice = $('#scprice').val();
			var $form_prvalidity = $('#prvalidity').val();
			var $form_owscprice = $('#owscprice').val();
			var $form_owotprice = $('#owotprice').val();
			var $form_owprvalid = $('#owprvalid').val();
			var $form_prscprice = $('#prscprice').val();
			var $form_protprice = $('#protprice').val();
			var $form_prprvalid = $('#prprvalid').val();
			var $form_retailflat = $('#retailflat').val();
			var $form_retailcent = $('#retailcent').val();
			var $form_retailslab = $('#retailslab').val();
			var $form_owflat = $('#owflat').val();
			var $form_owcent = $('#owcent').val();
			var $form_owslab = $('#owslab').val();
			var $form_prflat = $('#prflat').val();
			var $form_prcent = $('#prcent').val();
			var $form_prslab = $('#prslab').val();
