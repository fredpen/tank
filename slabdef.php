		...
		?>
			...
			<div id="items_in_cart">
				...
				?>
					...
					<input type="hidden" name="<?php echo 'lbound' . $slabdisc_var[$k]['lbound']; ?>" id="<?php echo 'lbound' . $slabdisc_var[$k]['lbound']; ?>" value="<?php echo $slabdisc_var[$k]['lbound']; ?>" />

					<input type="hidden" name="<?php echo 'ubound' . $slabdisc_var[$k]['lbound']; ?>" id="<?php echo 'ubound' . $slabdisc_var[$k]['lbound']; ?>" value="<?php echo $slabdisc_var[$k]['ubound']; ?>" />

					<input type="hidden" name="<?php echo 'disc' . $slabdisc_var[$k]['lbound']; ?>" id="<?php echo 'disc' . $slabdisc_var[$k]['lbound']; ?>" value="<?php echo $slabdisc_var[$k]['disc']; ?>" />

					<input type="hidden" name="<?php echo 'targetdisc' . $slabdisc_var[$k]['lbound']; ?>" id="<?php echo 'targetdisc' . $slabdisc_var[$k]['lbound']; ?>" value="<?php echo $slabdisc_var[$k]['targetdisc']; ?>" />
				<?php
				}
			...
			<table border="0" style="border:1px solid black;padding:1px;border-collapse:separate;border-radius:15px">
				<tr>
					...
					<td colspan="2">
						<input type="text" name="slabid" id="slabid" value="<?php echo $slabid; ?>" <?php if ($slabid != '') {
																										echo "style='color:gray'";
																										echo 'readonly';
																									} ?> placeholder="Enter Slab ID" />
					</td>
					<td align="right" nowrap="nowrap">

						<input type="button" name="refreshbutt" id="submit-button" value="Refresh" title="Click to Prepare for fresh record" onclick="javascript:  getpage('slabdef.php?','page');">
				...
				<tr>
					...
					<td>
						<b>Target Based Slab</b>&nbsp;&nbsp;<input title="Tick if this is a Target Based Slab" type="checkbox" id="iftarget" <?php if ($iftarget == 1) {
																																					echo 'checked';
																																				} ?> />
					</td>
				</tr>
...
<script>
	function mysavefunction() {
		if (confirm('Are you sure the entries are correct?')) {
			...
			if (Number($item_count) == 0) {
				$goahead = $goahead * 0;
				alert('Slab Discount Details Not Provided');
			}
			if ($form_slabid == '') {
				$goahead = $goahead * 0;
				alert('Specify Slab ID');
			}
			if ($form_slabdesc == '') {
				$goahead = $goahead * 0;
				alert('Specify Slab Description');
			}
	...
	function myAddItem() {
		...
		if ($form_lbound == '') {
			$addrow = $addrow * 0;
			alert('Specify Lower Bound Value');
		}
		if ($form_disc == '') {
			$addrow = $addrow * 0;
			alert('Specify Upper Bound Value');
		}
		if ($form_ubound == '') {
			$addrow = $addrow * 0;
			alert('Specify Discount');
		}
		if ($form_slabid == '') {
			$addrow = $addrow * 0;
			alert('Specify Slab ID');
		}
		if (Number($form_lbound) >= Number($form_ubound)) {
			$addrow = $addrow * 0;
