...
$sql_tanks = " SELECT distinct mainstorage as tankid FROM itemsubloc " .
	" WHERE TRIM(loccd) = '$loccd' order by mainstorage ";

$result_tanks = mysqli_query($_SESSION['db_connect'], $sql_tanks);
$numrows_tanks = mysqli_num_rows($result_tanks);
...
<div align="center" id="data-form">
	...
	<form name="form1" id="form1" method="post" action="">
		<h3 class="noprint"><strong>
				Tank Reading Consecutive Loses
			</strong></h3>
		...
		<div id="print_table">
			<h3><strong>
					Tank Reading Consecutive Losses
				</strong></h3>
			<h3><strong>Reporting from <?php echo $startdate; ?> &nbsp; to <?php echo $enddate; ?> </strong><?php if ($loccd == '') {
																												echo 'For No Location ';
																											} else {
																												echo ' for Location ' . $loccd;
																											} ?></h3>
				...
				<table width="95%" border="1">
					...
					?>
						...
						<tr <?php echo ($u % 2 == 0) ? "class='treven'" : "class='trodd'"; ?>>
							...
							<td nowrap="nowrap"><?php echo $dailytankarray[$u]['readingdate']; ?></td>
							<?php


							for ($v = 0; $v < $numrows_tanks; $v++) {
							?>
								<td nowrap="nowrap" class="Odd"><?php if (array_key_exists($u . trim($tankidarray[$v]), $dailytankarray)) {
																	echo $dailytankarray[$consecutive_array[$u] . $tankidarray[$v]];
																} ?></td>
