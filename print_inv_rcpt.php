...
<div  align ="center" id="data-form" > 
	...
	<form action="" method="get" id="form1">
		<h3 class= "noprint"><strong><font size='12'>Print Invoice or Receipt</font></strong></h3>
			...
			if ($_SESSION['prnrcpt']==1 || $_SESSION['prninv']==1)
			{
				...
				if($op=='searchclient')
					{
						...
						if ($searchin=="custno")
						{
							if($keyword!=''){$filter=" AND trim(custno) like '%$keyword%'   ";}
						}
						else
							{
								if($keyword!=''){$filter=" AND trim(company) like '%$keyword%'   ";}
							}
				...
				if ($op=='searchinvoiceno') 
					{
						...
						if($keyword_invoiceno!=''){$filter=" AND trim(invoice_no) like '%$keyword_invoiceno%'   ";}
						if (strlen($keyword_invoiceno)==0) {$sql_Q = $sql_Q . " and 1 = 2 ";}
				...
				if($op=='searchreceiptno')
				{
					...
					$sql_Q = "SELECT a.custno,a.refno,a.amount,a.descriptn,a.pmtdate,a.transby, b.company  
							...
							if($keyword_receiptno!=''){$filter=" AND trim(a.refno) like '%$keyword_receiptno%'   ";}
							if (strlen($keyword_receiptno)==0) {$sql_Q = $sql_Q . " and 1 = 2 ";}
				...
				<td  colspan="2" >
					<b>Select Invoice : </b>&nbsp;
						...
						<select name="invoice_no"   id="invoice_no" 
								onChange="javascript: whattoprint(this.id,this.value); "
						>
							<option  value="" ></option>
						<?php
						...
						<input type="submit" name="PrintInvoice" <?php  if (! empty($invoice_no)) {echo ' style="display:block;"';} else {echo ' style="display:none;"'; }?> class="PrintButton" id="submit-button" formtarget="_blank" value ="Print Invoice" formaction="<?php echo $_SESSION['applicationbase'].'printinvoice.php' ;?>"/>
				</td>
			</tr>
			<tr >
				<td  colspan="2" >
					...
					<b>Select Receipt : </b>&nbsp;
						...
						<input type="submit" <?php  if (! empty($refno)) {echo ' style="display:block;"';} else {echo ' style="display:none;"'; }?>  name="PrintReceipt" class="PrintButton" id="submit-button" formtarget="_blank" value ="Print Receipt" formaction="<?php echo $_SESSION['applicationbase'].'printreceipt.php'?>" />
				</td>
			</tr>
		</table>
