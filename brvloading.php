...
<div align ="center" id="data-form" > 
	...
	<form action="" method="get" id="form1" novalidate >
		<h3><strong><font size='10'>Way Bill</font></strong></h3>
		<?php
					...
					if ($count_get_invoice_header > 0)
					{
						...
						if ($goahead==1)
						{
							...
							{							
								...
								$theitem = $_REQUEST['theitem'.$i];
								$thesubinv = $_REQUEST['subloc'.$i];
								if (trim($rcvloc) != ''){ $rthesubinv = $_REQUEST['rsubloc'.$i];}else {$rthesubinv = '';}
			...
			<table width="90%" border="0">
				...
				<?php }  ?>
					<tr>
						...
						<td>
							<?php if (trim($rcvloc) != '') { echo '	<b>Receiving Location</b>';} ?><input type="hidden" id="rcvloc" value="<?php echo $rcvloc; ?>" />
						</td>
						<td>
							<?php if (trim($rcvloc) != '') { echo $rcvloc." " .$rcv_locnm;} ?>
						</td>
				</tr>
				<tr>
					<td>
		...
		<div id="loasection" style="display:none">
			...
			<table cellpadding="15px">
				...
				<tr>
					...
					<td><input type="text" size="7" id="mtr_b4"  value="<?php echo $mtr_b4;?>" 
							onChange="javascript: checkreadings(this.id);"
							/></td>
					<td align="right"> <b>Meter After</b></td>
					<td align="center"><input type="text" size="7" id="mtr_after"  value="<?php echo $mtr_after;?>"
						onChange="javascript: checkreadings(this.id); "
					/></td>
				</tr>
			</table>
		...
		<table>
		  <tr>
			...
			<td  align="right" nowrap="nowrap">
				  ...
				  <input type="button" name="approvebutton" id="submit-button" title="Generate Way Bill/Invoice" value="Gen.Waybill" 
					onclick="javascript: generateinvoice();">
			...
			<?php } ?>
				...
				<td>
					<?php $calledby = 'brvloading'; $reportid = 40; include("specificreportlink.php");  ?>
				</td>
		  </tr>
...
<script>
	...
	function checkreadings(thisid)
	{
		...
		if (Number(mtr_b4.value) >= Number(mtr_after.value)){resetmtr = 1;}
		if (Number(mtr_after.value) - (Number(mtr_b4.value) )> Number(store_qty.value)){alert('Meter Difference should not be more than quantity to Load');resetmtr = 1;}
	...
	function generateinvoice()
	{
		...
		if (confirm('Are you sure the entries are correct?')) {
			...
			var loccd = $('#loccd').val();var rcvloc = $('#rcvloc').val();var count_products = $('#count_products').val();
			...
			var upseal_no1 = $('#upseal_no1').val();var upseal_no2 = $('#upseal_no2').val();var upseal_no3 = $('#upseal_no3').val();var upseal_no4 = $('#upseal_no4').val();var upseal_no5 = $('#upseal_no5').val();var upseal_no6 = $('#upseal_no6').val();
			var loseal_no1 = $('#loseal_no1').val();var loseal_no2 = $('#loseal_no2').val();var loseal_no3 = $('#loseal_no3').val();var loseal_no4 = $('#loseal_no4').val();var loseal_no5 = $('#loseal_no5').val();var loseal_no6 = $('#loseal_no6').val();
