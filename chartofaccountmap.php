...
<div align ="center" id="data-form" > 
	...
	<form action="" method="get" id="form1">
	 <h3><strong><font size='10'>Chart Of Account Map </font></strong></h3> 
		<?php
			...
			if($op=='searchitem')
				{
					...
					{
						if($keyword!=''){$filter=" AND trim(themodule) like '%$keyword%'   ";}
					}else
						{
							if($keyword!=''){$filter=" AND trim(module_description) like '%$keyword%'   ";}
						}
			...
			<tr >
				<td colspan="2" >
					<b>Select Module</b>&nbsp;&nbsp;
						...
						&nbsp;&nbsp;&nbsp;
						<?php $calledby = 'chartofaccountmap'; $reportid = 57; include("specificreportlink.php");  ?>
						&nbsp;&nbsp;&nbsp;
					...
					<table border ="0">	
						...
						<tr>
							...
							<td>
								...
								<input type="button" name="additem" id="submit-button" value="Add or Modify" 
									onclick="javascript: addmodifymap();
										">
							</td>
						</tr>
						<tr>
...
<script>
	function addmodifymap()
	{
		if (confirm('Are you sure the entries are correct?')) 
		{
			...
			var $form_description = $('#description').val(); var $form_themodule = $('#themodule').val(); 
			var $form_credit_or_debit = $('#credit_or_debit').val();var $form_selectmodule = $('#selectmodule').val();
			...
			if ($goahead == 1) 
			{
				getpage('chartofaccountmap.php?op=additem&chartcode='+thechartcode
					...
					+'&themodule='+$form_themodule+'&credit_or_debit='+$form_credit_or_debit
					,'page');
			}
		}	
