			...
			<div style="overflow-x:auto;">
				<table border="0" style="border:1px solid black;padding:1px;border-collapse:separate;border-radius:15px">
					<tr>
						...
						<td>
							<input type="text" name="vendorno" id="vendorno" value="<?php echo $vendorno; ?>" <?php if ($vendorno != '') {
																													echo 'readonly';
				...
				<table>
					...
					<tr>
						...
						<td>
							...
							<input type="button" name="refreshbutton" title="Click to prepare form for a New Vendor creation" id="submit-button" value="Refresh" onclick="javascript:  getpage('vendors.php?','page');" />

						</td>
						<td>

							<input type="button" name="deletebutton" id="submit-button" value="Delete" onclick="deletevendor();" />
...
<script>
	function savevendor() {
		if (confirm('Are you sure the entries are correct?')) {
			...
			var $form_vendorno = $('#vendorno').val();
			...
			var $form_contactperson = $('#contactperson').val();
			var $form_phone = $('#phone').val();
			var $form_state = $('#state').val();
