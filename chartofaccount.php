...
<div align ="center" id="data-form_rptheader" > 
	...
	<form action="" method="get" id="form1">
		<h3><strong><font size='5'>Chart of Account </font></strong></h3>
		<?php
			...
			{
				$goahead=1;
				if (trim($_REQUEST['chartcode']) ==''){$goahead=0;
				...
				if (trim($_REQUEST['description']) ==''){$goahead=0;
				...
				if (trim($_REQUEST['financialstatement']) ==''){$goahead=0;
				...
				if (trim($_REQUEST['accounttype']) ==''){$goahead=0;
					?>	
...
//****		
		...
		<table border="0" style="border:1px solid black;padding:1px;border-collapse:separate;border-radius:15px">
			...
			<tr>
				...
				<td>
					<input type="text" name="chartcode" id="chartcode" value="<?php echo $chartcode; ?>" <?php if ($chartcode != ''){echo "style='color:lightgray'" ;echo 'readonly';} ?> class="required-text" />
				</td>
			</tr>
			<tr>
				<td  align="left">
		...
		<table>
			...
			<tr>
				...
				<td  colspan="3">
					...
					<?php $calledby = 'chartofaccount'; $reportid = 56; include("specificreportlink.php");  ?>	
			...
			<tr>
				<td>
					<input type="button" name="refreshbutton" id="submit-button" value="Refresh" 
						onclick="javascript:  getpage('chartofaccount.php?','page');" />
				</td>
				...
				<td>
					<input type="button" name="deleterecord" id="submit-button" value="Delete" 
						onclick="javascript:
							if (confirm('Are you sure of this action?')) {
									var $form_chartcode = $('#chartcode').val();
