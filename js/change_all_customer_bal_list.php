...
<?php 
			...
			$limit = !isset($_REQUEST['limit'])?"5000":$_REQUEST['limit'];
			...
			$filter = "";
			if($searchby!=''){$filter=" AND ACCT_TYPE = '$searchby' ";}
...
where account_details.BRANCH_ID=branch.branch_code and (account_details.ACCT_TYPE <> '' ) and ACCT_NAME <> '' ".$filter.$searchdate.$order;
	...
	$query = " Select ACCT_ID,  ACCT_NO, OLD_ACCT_NO, ACCT_NAME, GL_TYPE,ACCT_TYPE ACT,
(SELECT ACCT_TY_NAME from account_type_tbl where ACCT_TYPE_ID=account_details.ACCT_TYPE ) ACCT_TYPE,
(SELECT current_bal from current_balance where ACCT_NO=account_details.ACCT_NO ) CUS_BAL 
From
account_details, branch
...
<tr>
    ...
    <td colspan="2"><table border="0" cellspacing="1" class="table_text1" width="100%">
	        ...
	        <tr class="tr_whitebackground">
	          <td colspan="6" nowrap="nowrap"><table width="322" border="0">
                <tr>
                  <td align="left" nowrap="nowrap"><span class="table_text1" ><a href="#" onclick="javascript: goFirst('change_all_customer_bal_list.php');"><img src="/mfb/images/first.gif" alt="First" width="16" height="16" border="0"></a> <a href="#" onclick="javascript: goPrevious('change_all_customer_bal_list.php');"><img src="/mfb/images/prev.gif" alt="Previous" width="16" height="16" border="0"></a><a href="#" onclick="javascript: goNext('change_all_customer_bal_list.php');"><img src="/mfb/images/next.gif" alt="Next" width="16" height="16" border="0"/></a><a href="#" onclick="javascript: goLast('change_all_customer_bal_list.php');"><img src="/mfb/images/last.gif" alt="Last" width="16" height="16" border="0"/></a></span></td>
                  </tr>
              </table></td>
	          <td>&nbsp;</td>
          </tr>
	        ...
	        <tr class="tr_whitebackground">
	          <td colspan="7" nowrap="nowrap"><table width="688" border="0">
                <tr>
		  ...
		  <input name="search2" type="button" class="table_text1" disabled="disabled" id="search2" onclick="javascript:doSearch('change_all_customer_bal_list.php')" value="Search"/></td>
                  <td>&nbsp;</td>
                  <td>&nbsp;</td>
                </tr>
              </table></td>
	        ...
	        <tr class="back_image" >
	          <td colspan="7" align="left"><table border="0">
                <tr>
                  <td nowrap="nowrap">No of Records : <?php echo $count; ?></td>
			          <td>Limit :
			            <select name="limit" class="table_text1"  id="limit" onchange="javascript:doSearch('change_all_customer_bal_list.php');">
                  ...
                  <td>Page
		              ...
		              &nbsp;
		              <input name="search22" type="button" class="table_text1" id="search22" onclick="javascript:doSearch('change_all_customer_bal_list.php');" disabled="disabled" value="Go" /></td>
                </tr>
              </table></td>
          </tr>
	        <tr class="back_image" >
	          <td colspan="7" align="left"><font class='table_text1'><a href="user.php" >
	            <label>	            </label>
	          </a></font></td>
          </tr>
      </table></td>
  ...
  <tr>
    <td width="466" align="left"><div id="print_div">
      <table width="776"  border="0" cellpadding="5" cellspacing="1" class="menu_backcolor" id="accountinfo">
        <thead>
          ...
          <tr class="right_backcolor">
            <th width="1" nowrap="nowrap" class="Corner">&nbsp;</th>
                ...
                <th width="23" nowrap="nowrap">Rep Int</th>
                <th width="24" nowrap="nowrap">Change Balance</th>
                <?php } ?>
                </tr>
          </thead>
        <?php 
 		...
 		/* for($i=0; $i<$skip; $i++){
			...
			//for($i=0; $i<$numrows; $i++){
				...
				$all_a = $dbobject->getItemLabelArr('loan_tbl2',array('staff_no','loan_status'),array($row["ACCT_NO"],'1'),
        ...
        <tr <?php echo ($k%2==0)?"class='treven'":"class='trodd'"; ?> >
          <td nowrap="nowrap">&nbsp;</td>
                ...
                <td nowrap="nowrap"><?php echo number_format($row["CUS_BAL"],2); $tot_bal+=$row["CUS_BAL"];?></td>
                 <?php if(($searchby=='30') ||($searchby=='31') ||($searchby=='32') ||($searchby=='33') ||($searchby=='34') ||($searchby=='35') ||($searchby=='36') ||($searchby=='37')||($searchby=='38')){ ?>
                <td nowrap="nowrap"><?php echo number_format($all_a["outstd_rep"],2); $ln_oust+=$all_a["outstd_rep"];?></td>
                <td nowrap="nowrap"><?php echo number_format($all_a["outstd_int"],2); $Int_oust+=$all_a["outstd_int"];?></td>
                <td nowrap="nowrap"><?php echo number_format($all_a["total_rep"],2); $Rep_amt+=$all_a["total_rep"];?></td>
                <td nowrap="nowrap"><?php echo number_format($all_a["repay_int"],2); $Rep_int+=$all_a["repay_int"];?></td><?php }
				if($row["GL_TYPE"]!='20001'){$id2=$row["ACCT_ID"].'_'.$row["ACT"]; } ?>
                <td nowrap="nowrap"><input type="text" id="<?php echo $id2;?>" name="<?php echo $id2;?>" value="<?php echo $row["CUS_BAL"];?>" />
                  <input name="search" type="button" class="table_text1" id="search" 
                onclick="javascript:getpageUpdateGl('<?php echo $id2;?>','change_all_customer_bal_list.php','page')" value="Update"/></td>
  ...
  <tr>
    <td align="left"><table width="322" border="0">
      <tr>
