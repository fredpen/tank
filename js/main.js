function checklogin(formurl) {
  ...
  for (var i = 0; i < themodule.length; i++) {
    if (themodule[i].checked) data3 = themodule[i].value;
  }
  ...
  if (data == "" || data2 == "") {
    $("#error_label_login").html(
      '<img src="images/loading.gif" alt="" />Enter log in details . . .'
    );
    return false;
  }
  //alert(data+":"+data2);
  ...
  $.ajax({
    ...
    success: function (msg) {
      ...
      if (msg == "") {
        ...
        $("#error_label_login")
          .html("You do not have access to selected Module")
          .show();
      } else if (msg == "1") {
        //$("#member_data").val(msg);
